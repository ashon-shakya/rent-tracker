          ...
          <tbody className="divide-y divide-slate-50">
            <tr className="hover:bg-violet-50/30 transition-colors">
              ...
              <td className="px-4 py-4">
                <div className="flex gap-1 flex-wrap">
                  <span className="text-[10px] font-bold text-violet-600 bg-violet-50 px-2 py-1 rounded-full">R: {rent.adminRentShareParts ?? 1}p</span>
                  <span className="text-[10px] font-bold text-violet-600 bg-violet-50 px-2 py-1 rounded-full">B: {rent.adminBondShareParts ?? 1}p</span>
                </div>
              </td>
              <td className="px-4 py-4 text-right">
                {isAdmin && (
                  <div className="flex justify-end gap-1">
                    <button 
                      onClick={() => handleEditClick({ _id: 'admin', name: 'Owner', rentShareParts: rent.adminRentShareParts ?? 1, bondShareParts: rent.adminBondShareParts ?? 1 })}
            ...
            {tenants.map((t: any) => (
              <tr key={t._id} className="hover:bg-violet-50/30 transition-colors">
                ...
                <td className="px-4 py-4">
                  <div className="flex gap-1 flex-wrap">
                    <span className="text-[10px] font-bold text-slate-500 bg-slate-50 px-2 py-1 rounded-full">R: {t.rentShareParts}p</span>
                    <span className="text-[10px] font-bold text-sky-500 bg-sky-50 px-2 py-1 rounded-full">B: {t.bondShareParts}p</span>
                  </div>
                </td>
                <td className="px-4 py-4 text-right">
                  {isAdmin && (
        ...
        <div className="space-y-2">
          ...
          <div className="bg-[#f8f7fc] rounded-xl px-4 py-3 flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              ...
              <span className="text-xs font-bold text-violet-600">Bond: {rent.adminBondShareParts ?? 1}p</span>
            </div>
          </div>
        </div>
        {tenants.map((t: any) => (
          <div key={t._id} className="space-y-2">
            ...
            <div className="bg-[#f8f7fc] rounded-xl px-4 py-3 flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <span className="text-xs font-bold text-slate-500">Rent: {t.rentShareParts}p</span>
                <span className="text-xs font-bold text-sky-500">Bond: {t.bondShareParts}p</span>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] w-full max-w-md shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-[#f8f7fc]/50">
              ...
              <button onClick={closeEdit} className="w-8 h-8 flex items-center justify-center rounded-full bg-white text-slate-400 hover:text-slate-600 shadow-sm transition-colors">
                <X size={18} />
              </button>
            </div>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    step="0.01"
                    min="0"
                    defaultValue={editingTenant.rentShareParts}
                    required
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    step="0.01"
                    min="0"
                    defaultValue={editingTenant.bondShareParts}
                    required
